...
var fn_start_page = () => {
    ...
    h_manager.grid.fn_create_grid(grid.gid, {
        g_Headers : ["표제부 PK", "건물구분", "용도코드", "기타용도", "지번주소", "건물 명", "호실 수", "세대수", "가구수", "주차대수"
                    , "엘리베이터 수"
         ]
        , g_Models: [
...
var fn_search = (pgno) => {
    ...
    h_manager.network.post("/buldapi/bapi/0401", {
        "arcd"   : p_arcd,
        "legcd"  : p_legcd, 
        "bunjib" : p_bunjib,
        "bunjij" : p_bunjij,
...
var fn_create_Ledgr = () => {
    ...
    var p_bunjib = $("#p_bunjib").val() ;
    var p_bunjij = $("#p_bunjij").val() ; 
    h_manager.network.post("/buldapi/bapi/0402", {
        "arcd"   : p_arcd,
        "legcd"  : p_legcd, 
        "bunjib" : p_bunjib,
        "bunjij" : p_bunjij
        ...
        h_manager.message.alert( response.data.message, {
            "title"  : "<strong>[ 홈즈관리대장-마스터 생성 ]</strong>", 
            "msgType": "success"
        })
    }) ;
}
